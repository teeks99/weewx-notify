...
def package_dir():
    return f"weewx-notify_{version}"
...
def copy_scripts():
    shutil.copytree("bin/user", os.path.join(dist_path, package_dir(), "bin/user"))
...
def file_paths():
    ...
    paths = []
    for root, _, files in os.walk("bin/user"):
        for filename in files:
            paths.append(os.path.join(root, filename))

    os.chdir(original_dir)

    return ("bin/user", paths)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Package the notify extension')
